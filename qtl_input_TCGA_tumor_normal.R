...
library(sas7bdat)
source("functions.R")
#Genotypes are from normals
folder="/fh/fast/stanford_j/Xiaoyu/QTL/result/imputation3"
pre="SNP6_info_"
...
for (chr in 1:23)
{
  ...
  filterimput(chr,imputfiles,infofiles,colscore=7,colmaf=6,scorecutoff=0.3,mafcutoff=0.05,outfolder="/fh/fast/stanford_j/Xiaoyu/QTL/result/filtered_imputation",
              prefix="TCGA_allnormals")
}


#SNP_GE,SNP_ME and SNP_POS

genereate_SNPfiles1=function(chrs=1:23,genexp_samples=hutch_geneexp_samples,methylation_samples=hutch_methylation_samples,
                             genotype_samples=colnames(all496normalgenotypedata),rdfiles,
                            colrsid=5,colpos=6,outfoler="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input",prefix="HUTCH")
{
  ...
  for (chr in chrs)
  {
    ...
    load(rdfiles[chr])
    imputetable=as.data.frame(imputetable)
    colnames(imputetable)[2:ncol(imputetable)]=genotype_samples #add this
...
normal_geneexpdata=geneexpdata[,!colnames(geneexpdata) %in% seminal_sampleid_bio]
normal_methydata=methydata[,!colnames(methydata) %in% seminal_sampleid_bio]
load("/fh/fast/stanford_j/Xiaoyu/QTL/data/TCGAtumors.RData")
tumor_geneexpdata=geneexpdata[,!colnames(geneexpdata) %in% seminal_sampleid_bio]
tumor_methydata=methydata[,!colnames(methydata) %in% seminal_sampleid_bio]

tcga_tumors_geneexp_samples=intersect(colnames(all496normalgenotypedata),colnames(tumor_geneexpdata))
tcga_tumors_geneexp_samples=tcga_tumors_geneexp_samples[!tcga_tumors_geneexp_samples %in% colnames(normal_geneexpdata)]
tcga_tumors_methy_samples=intersect(colnames(all496normalgenotypedata),colnames(tumor_methydata))
tcga_tumors_methy_samples=tcga_tumors_methy_samples[!tcga_tumors_methy_samples %in% colnames(normal_methydata)]
idx=match(tcga_tumors_geneexp_samples,colnames(geneexpdata))
tumor_geneexpdata=geneexpdata[,idx]
idx=match(tcga_tumors_methy_samples,colnames(methydata))
tumor_methydata=methydata[,idx]

tcga_normals_geneexp_samples=intersect(colnames(all496normalgenotypedata),colnames(normal_geneexpdata))
tcga_normals_methy_samples=intersect(colnames(all496normalgenotypedata),colnames(normal_methydata))

idx=match(tcga_tumors_geneexp_samples,colnames(tumor_geneexpdata))
GE=tumor_geneexpdata[,idx]
GE=removeconstrows(dat=GE)
GE=t(scale(t(GE)))
tumor_GE=GE
tmp=cbind.data.frame(id=rownames(GE),GE)
write.table(tmp,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_GE.txt",col.names = T,row.names = F,sep="\t",quote=F)
idx=match(tcga_normals_geneexp_samples,colnames(normal_geneexpdata))
GE=normal_geneexpdata[,idx]
GE=removeconstrows(dat=GE)
GE=t(scale(t(GE)))
normal_GE=GE
tmp=cbind.data.frame(id=rownames(GE),GE)
write.table(tmp,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_GE.txt",col.names = T,row.names = F,sep="\t",quote=F)
...
write.table(GE_POS,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_GE_POS.txt",col.names = T,row.names = F,sep="\t",quote=F)
write.table(GE_POS,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_GE_POS.txt",col.names = T,row.names = F,sep="\t",quote=F)

#ME
idx=match(tcga_normals_methy_samples,colnames(normal_methydata))
normal_ME=normal_methydata[,idx]

idx=match(tcga_tumors_methy_samples,colnames(tumor_methydata))
tumor_ME=tumor_methydata[,idx]
length(intersect(tcga_tumors_geneexp_samples,tcga_tumors_methy_samples))
#[1] 382


#remove all NA rows of me
process_ME=function(ME,methy_samples=tcga_normals_methy_samples)
{
  numNA=rep(0,nrow(ME))
  for (i in 1:nrow(ME))
  {
  ...
  ME1=ME1[idx,]
  idx1=match(methy_samples,colnames(ME1))
  ...
  return(list(ME=tmp,idx1=idx1))
}
tmp=process_ME(ME=normal_ME)
write.table(tmp$ME,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_ME.txt",col.names = T,row.names = F,sep="\t",quote=F)
HUTCH_ME_POS=fread("/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/HUTCH_ME_POS.txt")
write.table(HUTCH_ME_POS[tmp$idx1,],file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_ME_POS.txt",col.names = T,row.names = F,sep="\t",quote=F)
tmp=process_ME(ME=tumor_ME,methy_samples = tcga_tumors_methy_samples)
write.table(tmp$ME,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_ME.txt",col.names = T,row.names = F,sep="\t",quote=F)
write.table(HUTCH_ME_POS[tmp$idx1,],file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_ME_POS.txt",col.names = T,row.names = F,sep="\t",quote=F)

#SNP_GE,SNP_ME,SNP_POS
rdfiles=paste0("/fh/fast/stanford_j/Xiaoyu/QTL/result/filtered_imputation/TCGA_allnormals_chr",1:23,".RData")
genereate_SNPfiles1(genexp_samples=tcga_normals_geneexp_samples,methylation_samples=tcga_normals_methy_samples,rdfiles=rdfiles,
                    colrsid=2,colpos=3,outfoler="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input",prefix="TCGA_normalsonly")
genereate_SNPfiles1(chrs=1:23,genexp_samples=tcga_tumors_geneexp_samples,methylation_samples=tcga_tumors_methy_samples,rdfiles=rdfiles,
                            colrsid=2,colpos=3,outfoler="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input",prefix="TCGA_tumorsonly")

normal_GE_PCA=pca(dat=normal_GE)#14,0.44
write.table(normal_GE_PCA,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_GE_PCA.txt",col.names = T,row.names = F,sep="\t",quote=F)
tumr_GE_PCA=pca(dat=tumor_GE)#14,0.44
write.table(tumor_GE_PCA,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_GE_PCA.txt",col.names = T,row.names = F,sep="\t",quote=F)
...
#SNP_PCA
TCGA_normals_SNP_GE=fread("/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_SNP_GE.txt",header=T,drop="id",sep="\t",stringsAsFactors = F)
TCGA_normals_SNP_GE_PCA=pca(dat=TCGA_normals_SNP_GE,check=T,bigpca = T,numpc=15)
write.table(TCGA_normals_SNP_GE_PCA,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_SNP_GE_PCA.txt",col.names = T,row.names = F,sep="\t",quote=F)
TCGA_tumors_SNP_GE=fread("/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_SNP_GE.txt",header=T,drop="id",sep="\t",stringsAsFactors = F)
TCGA_tumors_SNP_GE_PCA=pca(dat=TCGA_tumors_SNP_GE,check=T,bigpca = T,numpc=15)
write.table(TCGA_tumors_SNP_GE_PCA,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_SNP_GE_PCA.txt",col.names = T,row.names = F,sep="\t",quote=F)
rm(TCGA_tumors_SNP_GE)
TCGA_normals_SNP_ME=fread("/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_SNP_ME.txt",header=T,drop="id",sep="\t",stringsAsFactors = F)
TCGA_normals_SNP_ME_PCA=pca(dat=TCGA_normals_SNP_ME,check=T,bigpca = T,numpc=15)
write.table(TCGA_normals_SNP_ME_PCA,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_SNP_ME_PCA.txt",col.names = T,row.names = F,sep="\t",quote=F)
TCGA_tumors_SNP_ME=fread("/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_SNP_ME.txt",header=T,drop="id",sep="\t",stringsAsFactors = F)
TCGA_tumors_SNP_ME_PCA=pca(dat=TCGA_tumors_SNP_ME,check=T,bigpca = T,numpc=15)
write.table(TCGA_tumors_SNP_ME_PCA,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_SNP_ME_PCA.txt",col.names = T,row.names = F,sep="\t",quote=F)
...
GE_normal_PEER=peer_number(dat=normal_GE)#15,0.605
write.table(GE_normal_PEER,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_normalsonly_GE_PEER.txt",col.names = T,row.names = F,sep="\t",quote=F)
GE_tumor_PEER=peer_number(dat=tumor_GE)#15,0.459
write.table(GE_tumor_PEER,file="/fh/fast/stanford_j/Xiaoyu/QTL/result/qtl_input/TCGA_tumorsonly_GE_PEER.txt",col.names = T,row.names = F,sep="\t",quote=F)
